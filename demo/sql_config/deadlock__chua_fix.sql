-- ==========================================================
-- deadlock__chua_fix.sql — SP_DangKyNhieuHocPhan BẢN CỐ Ý CÓ LỖI
-- BẢN COPY DÙNG CHO DEMO (thêm DO SLEEP để mở rộng cửa sổ khóa)
-- ❌ LỖI: con trỏ khóa theo ĐÚNG thứ tự sinh viên tick chọn => đảo thứ tự là deadlock 1213
-- ==========================================================
...
proc_dk_nhieu_chuafix: BEGIN
    ...
    DECLARE vDoTreGiay  INT DEFAULT 2;      -- ⏸ giây giữ khóa dòng ĐẦU TIÊN (mở rộng cửa sổ deadlock)
    DECLARE vLanKhoa    INT DEFAULT 0;      -- đếm số lần đã khóa
    ...
    dk_loop: LOOP
        ...
        -- ⏸ SAU KHI đã giữ khóa dòng đầu tiên: giữ thêm vài giây để phiên kia
        --    kịp khóa dòng của nó rồi quay lại đòi dòng này => hình thành chu trình chờ.
        SET vLanKhoa = vLanKhoa + 1;
        IF vLanKhoa = 1 AND vDoTreGiay > 0 THEN
            DO SLEEP(vDoTreGiay);
        END IF;
