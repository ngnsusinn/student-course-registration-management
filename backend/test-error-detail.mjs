...
async function main() {
  ...
  // Apply buggy SP and reset
  await apFileSql('lost_update_chua_fix');
  // Reset via direct DB call
  ...
  // Check error detail for B
  console.log('=== Test 2 concurrent web requests ===');
  const t1 = Date.now();
  ...
  console.log(`Request A: HTTP ${r1.status}`);
  console.log(`  ketQua: ${r1.data.ketQua}`);
  console.log(`  message: ${r1.data.message}`);
  if (r1.data.error) console.log(`  error: ${r1.data.error}`);
  
  console.log(`\nRequest B: HTTP ${r2.status}`);
  console.log(`  ketQua: ${r2.data.ketQua}`);
  console.log(`  message: ${r2.data.message}`);
  if (r2.data.error) console.log(`  error: ${r2.data.error}`);
  
  console.log(`\nTotal time: ${Date.now()-t1}ms`);
