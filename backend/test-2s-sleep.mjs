// Test web concurrent with 2-second sleep
...
async function main() {
  ...
  // Re-apply with 2-second sleep
  await apFileSql('lost_update_chua_fix');
  console.log('✅ Đã apply lost_update_chua_fix (2s sleep)');
  ...
  console.log(`\nSingle request: HTTP ${single.status}, ketQua=${single.data.ketQua}`);
  ...
  console.log(`Request A: HTTP ${r1.status}, ketQua=${r1.data.ketQua}`);
  console.log(`Request B: HTTP ${r2.status}, ketQua=${r2.data.ketQua}`);
  ...
  const [lhp] = await conn3.query("SELECT SiSoHienTai, SiSoToiDa, (SELECT COUNT(*) FROM DANGKYHOCPHAN WHERE MaLHP='LHP506' AND TrangThaiDangKy='DA_DANG_KY') AS SoDK FROM LOPHOCPHAN WHERE MaLHP='LHP506'");
